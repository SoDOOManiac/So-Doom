...
*/
    ...
    for (i=0;i<9;i++)
    {
    if (cht_CheckCheatSP (&(cheat_weapon[i+1]), ev->data2) || cht_CheckCheatSP (&(cheat_weapon2[i+1]), ev->data2))
    {
    // [crispy] only give available weapons
	...
	else
	{
		...
		P_GiveWeapon(plyr, i, false);
		S_StartSound(NULL, sfx_wpnup);

		if (i > 1)
		{
		    plyr->message = DEH_String(WeaponPickupMessages[i]);
		}
    ...
    {
	    M_snprintf(msg, sizeof(msg), "Weapon %s%d%s %s",
	               crstr[CR_GOLD], i + 1, crstr[CR_NONE],
	               plyr->weaponowned[i] ? "added" : "removed");
	    plyr->message = msg;
    }
    }
    }
...
void ST_drawWidgets(boolean refresh)
{
    ...
    {
    dp_translucent = true;
    V_DrawPatch(23 - SHORT(faceback_sd[displayplayer]->width)/2-(crispy->widescreen ? 1 : 0)*ST_WIDESCREENDELTA, ST_Y - ST_HEIGHT, faceback_sd[displayplayer]);
    dp_translucent = false;
    }
    V_DrawPatch(23 - SHORT(faceback_sd[displayplayer]->width)/2-(crispy->widescreen ? 1 : 0)*ST_WIDESCREENDELTA, ST_Y-ST_HEIGHT, faces[st_faceindex]);
    }
...
void ST_Init (void)
{
    ...
    ST_loadData();
    st_backing_screen = (pixel_t *) Z_Malloc(MAXWIDTH * (ST_HEIGHT << 2) * sizeof(*st_backing_screen), PU_STATIC, 0);
}
