...
// [So Doom] apply delta to Crispy HUD or no status bar (for Status Bar level stats position) if screenblocks >= CRISPY_HUD + 4, i.e. HUD is wide
#define ST_WIDESCREENDELTA ((screenblocks >= CRISPY_HUD + 4 && (!automapactive || crispy->automapoverlay)) ? WIDESCREENDELTA : 0)
